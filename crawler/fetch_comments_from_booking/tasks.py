...
from crawler.utils.fetch_data import \
    get_comments_from_booking  # Function to extract and format comments from the Booking.com API response


# Define an asynchronous task using the shared_task decorator provided by Celery
@shared_task
def fetch_comments(skip, hotel_id, ufi, hotel_country_code):
    """
    Asynchronous task to fetch comments for a specific hotel on Booking.com.
    ...
    Returns:
        list: A list of formatted comments extracted from the Booking.com API response.
    """
    ...
    # Extract and format the comments from the JSON response using the get_comments_from_booking function.
    comments = get_comments_from_booking(response.json())
